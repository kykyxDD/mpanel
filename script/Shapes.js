...
(lib.Shapes = function(mode,startPosition,loop) {
	...
	this.frame_0 = function() {
		...
		var self = this;
		var previewWidth = 430;//500;
		var previewHeight = 430;//500;
		var previewPaddings = 130;//105;
		var preview; 
		var lines;
		var lettersArr;
		var letterSize  = 26;
		...
		this.updateShape=function(obj, animation){
			...
			if(res){
				for(var i=0; i<currentObject.length;i++){
					if(animation){
						createjs.Tween.get(currentObject[i]).to({val:obj[i].val, curved:obj[i].curved}, 500, createjs.Ease.cubicInOut);
					}else{
						currentObject[i].val = obj[i].val;
						currentObject[i].curved = obj[i].curved;
					}
				}
