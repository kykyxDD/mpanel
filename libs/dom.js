...
dom.addclass = function(elem, name) {
	...
	elem.className = f.sor(left, right).filter(Boolean).join(' ')
}
...
dom.remclass = function(elem, name) {
	...
	elem.className = f.snot(left, right).filter(Boolean).join(' ')
}
...
dom.hasclass = function(elem, name, one) {
	...
	var found = f.sand(left, right).filter(Boolean).length
...
dom.scrollOffset = function(elem) {
	for(var x = 0, y = 0; elem; elem = elem.offsetParent) {
		x += elem.scrollLeft || 0
		y += elem.scrollTop  || 0
	}
